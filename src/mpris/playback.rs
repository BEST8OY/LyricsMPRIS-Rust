...
pub async fn get_position(service: &str) -> Result<f64, MprisError> {
    ...
    let position: Option<i64> = Properties::get(&proxy, "org.mpris.MediaPlayer2.Player", "Position").await.ok();
...
pub async fn get_playback_status(service: &str) -> Result<String, MprisError> {
    ...
    let conn = get_dbus_conn().await?;
    let proxy = Proxy::new(service, "/org/mpris/MediaPlayer2", TIMEOUT, conn);
    let playback_status: Option<String> = Properties::get(&proxy, "org.mpris.MediaPlayer2.Player", "PlaybackStatus").await.ok();
