...
// progression helpers were moved to `ui::progression` and are imported where needed.
use tui::{
    backend::Backend,
...
// progression timing helpers are provided by `ui::progression`.
...
// estimate_update_and_next_sleep is implemented in `ui::progression` and
// imported above for use by callers.
